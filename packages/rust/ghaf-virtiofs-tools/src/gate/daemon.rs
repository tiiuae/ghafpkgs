...
use crate::scanner::{ScanResult, VirusScanner};
use crate::util::{InfectedAction, notify_error, notify_infected, wait_for_shutdown};
use crate::watcher::{EventHandler, FileId, Watcher, WatcherConfig};
...
/// Permission mask to strip SUID (0o4000), SGID (0o2000), and sticky bit (0o1000).
const PERMISSION_MASK: u32 = 0o0777;
...
impl Daemon {
    ...
    /// Run the daemon, spawning channel runners and handling shutdown.
    ///
    /// # Errors
    /// Returns an error if shutdown signal handling fails.
...
impl<'a> ChannelRunner<'a> {
    fn new(
        ...
        let handler = ChannelHandler {
            ...
            scanner,
            notifier,
            share_base: config.base_path.join("share"),
            export: config.base_path.join("export"),
            staging: config.base_path.join("staging"),
            quarantine: config.base_path.join("quarantine"),
        };
...
impl EventHandler for ChannelHandler<'_> {
    fn on_modified(&mut self, path: &Path, source: &str) -> Vec<(FileId, i64)> {
        let share_path = self.share_base.join(source);
        ...
        if src_meta.len() == 0 {
            ...
            let written = self.propagate_clean(tmp.as_file(), &src_meta, source, rel);
            self.spawn_notify();
            return written;
        }
        ...
        self.dispatch_scan_result(scan_result, &tmp, &src_meta, path, rel, source)
    }
    ...
    fn on_deleted(&mut self, path: &Path, source: &str) {
        ...
        // Diode deletions don't propagate (diode is write-only, one-direction sync)
        if self.config.is_diode(source) {
            debug!(
                "Channel '{}': ignoring deletion from diode '{}'",
                self.name, source
            );
            return;
        }

        // Delete from other producers if exists (skip diode producers)
        for producer in &self.config.producers {
            if producer == source || self.config.is_diode(producer) {
                continue;
            }
            let target = self.share_base.join(producer).join(rel);
            if target.exists() {
        ...
        if !self.config.consumers.is_empty() {
            let export_target = self.export.join(rel);
            if export_target.exists() {
                let result = if export_target.is_dir() {
                    fs::remove_dir_all(&export_target)
                } else {
    ...
    fn on_renamed(&mut self, path: &Path, old_path: &Path, source: &str) -> Vec<(FileId, i64)> {
        ...
        // Diode renames don't propagate (diode is write-only, one-direction sync)
        // The old name stays in other producers/export
        if self.config.is_diode(source) {
            debug!(
                "Channel '{}': ignoring rename from diode '{}'",
                self.name, source
            );
            return vec![];
        }
        ...
        for producer in &self.config.producers {
            ...
            let old_target = self.share_base.join(producer).join(old_rel);
        ...
        if !self.config.consumers.is_empty() {
            let old_export = self.export.join(old_rel);
        ...
        let written = self.propagate_clean(&src_file, &src_meta, source, rel);
...
impl ChannelHandler<'_> {
    /// Dispatch scan result: propagate clean files, quarantine infected ones.
    ...
    fn verify_reflink_support(&self) -> Result<()> {
        ...
        let src = NamedTempFile::new_in(&self.staging).context("failed to create test file")?;
        ...
        let targets = self
            ...
            .producers
            .iter()
            .map(|producer| self.share_base.join(producer))
            .chain((!self.config.consumers.is_empty()).then(|| self.export.clone()))
            .chain(std::iter::once(self.quarantine.clone()));
    ...
    fn verify_chown_capability(&self) -> Result<()> {
        ...
        let tmp = NamedTempFile::new_in(&self.staging).context("failed to create test file")?;
    ...
    fn clone_to_staging(&self, src_file: &File) -> Result<NamedTempFile> {
        ...
        let tmp = NamedTempFile::new_in(&self.staging).context("failed to create staging file")?;
    ...
    /// Propagate a clean file to other producers and export.
    ...
    fn propagate_clean(
        ...
        for producer in &self.config.producers {
            ...
            let target = self.share_base.join(producer).join(src_relative);
        ...
        if !self.config.consumers.is_empty() {
            let export_target = self.export.join(src_relative);
    ...
    fn atomic_reflink(src_file: &File, src_meta: Option<&fs::Metadata>, dst: &Path) -> Result<()> {
        ...
        let (mode, uid, gid) = src_meta.map_or((0o000, 0, 0), |m| {
            (m.mode() & PERMISSION_MASK, m.uid(), m.gid())
        });
    ...
    fn handle_infected(&self, tmp: &NamedTempFile, src_path: &Path, relative: &Path) {
        match self.config.scanning.infected_action {
            ...
            InfectedAction::Quarantine => {
                let quarantine_dest = self.quarantine.join(relative);
...
mod tests {
    ...
    fn make_config_with_patterns(
        ...
        use super::super::config::{
            ChannelConfig, ScanningConfig, UserNotifyConfig, default_debounce_ms,
        };
    ...
    fn make_handler(config: &super::super::config::ChannelConfig) -> ChannelHandler<'_> {
        use crate::scanner::{ClamAVScanner, DEFAULT_CLAMAV_SOCKET};
        use std::sync::Arc;

        ChannelHandler {
            name: "test",
            config,
            scanner: Arc::new(ClamAVScanner::new(DEFAULT_CLAMAV_SOCKET)),
