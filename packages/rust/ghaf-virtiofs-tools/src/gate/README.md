...
**Overwrite Protection:** Diode producers use an "ignore-existing" strategy (similar to rsync `--ignore-existing`). If a file already exists at the target, the diode file is skipped. This prevents data loss when a diode producer unknowingly creates a file that already exists, and prevents "take-backs".

#### Propagation Matrix

The matrix shows how events propagate to other locations. The source file itself is always updated (that's the event that triggered propagation).
...
| Diode | Delete | Skip | Skip | Skip |
| Diode | Rename | Skip | Skip | Skip |
...
Diode delete/rename events are not propagated to prevent untrusted VMs from destroying files in trusted locations.
...
- **Watcher** (`Watcher` struct): inotify-based file system watcher with debouncing. Handles `CLOSE_WRITE`, `DELETE`, `MOVED_FROM`, `MOVED_TO`, and `CREATE` events. Can be swapped with a different watcher implementation (e.g., fanotify-based).

- **Notifier** (`Notifier` struct): Sends channel notifications to guest VMs over vsock to trigger file browser refresh. For compatibility with all file browsers it uses a toggled trigger file, as some file browsers do not refresh the file tree upon touches or metadata updates, and implement debouncing.
...
| Guest notification fails | CID skipped for 10s, warning rate-limited |
...
| `debounceMs` | number | `300` | Wait time after last write |
...
The notifier includes protection against resource waste and log spam when guest VMs are unreachable:

| Mechanism | Duration | Description |
| --------- | -------- | ----------- |
| CID Backoff | 10s | After a connection failure, skip that CID for 10 seconds |
| Rate-limited Warnings | 10s | Log at most one warning per CID per backoff period |
| Debouncing | 1s | Skip duplicate notifications to the same CID/channel within 1 second |

**CID Backoff**: When a connection to a guest VM fails (e.g., VM is not running, vsock not configured), the notifier skips that CID for 10 seconds. This prevents wasting resources on repeated connection attempts to unreachable VMs.

**Rate-limited Warnings**: Instead of logging every failed connection attempt, warnings are rate-limited to once per backoff period per CID. This prevents log spam when a VM is offline for extended periods.

**Debouncing**: When multiple file events occur in rapid succession (e.g., batch file operations), the notifier sends at most one notification per second per channel/CID combination. The guest VM will refresh and see all changes.

On successful connection, backoff and warning state is cleared, allowing immediate notifications to resume.
