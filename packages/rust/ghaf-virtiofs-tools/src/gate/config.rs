...
use crate::util::{InfectedAction, REFRESH_TRIGGER_FILE};
...
impl ChannelConfig {
    ...
    ///
    /// # Errors
    /// Returns `Err(Vec<String>)` containing validation error messages if:
    /// - Required directories are missing or inaccessible
    /// - No producers are defined
    /// - Diode producers are not in the producers list
    ...
    ///
    /// # Errors
    /// Returns an error if the config file cannot be read, parsed, or has invalid channels.
...
mod tests {
    ...
    fn test_default_values() {
        ...
        assert_eq!(channel.debounce_ms, default_debounce_ms());
        assert!(channel.scanning.enable);
