...
fn compute_producer_action(
    ...
    // Find the entry with latest mtime (conflict resolution) - keep reference, clone only if needed
    let (best_source, best_info) = entries.iter().max_by_key(|(_, info)| info.mtime)?;
    ...
    if producers_in_conflict || needs_producer_sync || needs_export_sync {
        Some(SyncAction::TriggerSync {
            source: best_source.clone(),
            path: best_info.path.clone(),
...
mod tests {
    ...
    fn make_test_config_with_diode(
        ...
        use super::super::config::{ScanningConfig, UserNotifyConfig, default_debounce_ms};
