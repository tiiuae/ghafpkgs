...
use ghaf_virtiofs_tools::scanner::{
    ClamAVScanner, DEFAULT_CLAMAV_SOCKET, ScanResult, VirusScanner,
};
...
/// Set high enough to handle burst loads (e.g., archive extraction)
/// without dropping scan requests. Memory cost is ~124 bytes per entry.
const SCAN_CHANNEL_CAPACITY: usize = 1_000;

/// Maximum concurrent scans. Bounded to avoid overwhelming `ClamAV` daemon
/// (which has its own `MaxThreads` limit, typically 10-20).
...
/// Chunk size for streaming files to scanner (must be < vproxy's `max_chunk_size` of 25MB)
const STREAM_CHUNK_SIZE: usize = 16 * 1024 * 1024;
...
struct Cli {
    ...
    /// `ClamAV` daemon socket path (only used with --socket)
...
async fn main() -> Result<()> {
    ...
    if cli.socket {
        wait_for_local_scanner(&cli.clamd_socket).await?;
    } else {
...
async fn wait_for_local_scanner(socket_path: &str) -> Result<()> {
    let scanner = ClamAVScanner::new(socket_path);
    let mut logged_waiting = false;

    loop {
        if scanner.validate_availability().is_ok() {
...
async fn run(cli: Cli) -> Result<()> {
    ...
    // Shared state for parallel scan tasks
    let semaphore = std::sync::Arc::new(Semaphore::new(MAX_CONCURRENT_SCANS));
    let quarantine_dir: Option<std::sync::Arc<Path>> = cli
        .quarantine_dir
        .map(|p| std::sync::Arc::from(p.as_path()));
    let notify_socket: std::sync::Arc<Path> = std::sync::Arc::from(cli.notify_socket.as_path());
    let clamd_socket: std::sync::Arc<str> = std::sync::Arc::from(cli.clamd_socket.as_str());
    let cid = cli.cid;
    let port = cli.port;
    let action = cli.action;
    let use_socket = cli.socket;

    // Spawn scanner dispatcher that spawns bounded parallel scan tasks
        ...
        loop {
            tokio::select! {
                Some(path) = rx.recv() => {
                    ...
                    let notify = notify_socket.clone();
                    let socket = clamd_socket.clone();

                    tasks.spawn(async move {
                        // Acquire permit (waits if at max concurrency)
                        let _permit = sem.acquire().await.expect("semaphore closed");

                        let result = if use_socket {
                            let p = path.clone();
                            tokio::task::spawn_blocking(move || scan_file_socket(&p, &socket))
                                .await
                        ...
                        match result {
                            Ok(scan_result) => {
                                handle_scan_result(&path, &scan_result, action, qdir.as_deref(), &notify);
                            }
                            Err(e) => {
                                error!("Scan error for {}: {e}", path.display());
                                notify_error(&notify, &path, &e.to_string());
                            }
                        }
                    });
                }
                // Poll completed tasks to free resources
...
fn scan_file_socket(path: &Path, socket_path: &str) -> Result<ScanResult> {
    debug!("Scanning: {}", path.display());
    ClamAVScanner::new(socket_path).scan_path(path)
}
...
async fn scan_file_vsock(path: &Path, cid: u32, port: u32) -> Result<ScanResult> {
    ...
    debug!("Scanning: {} ({} bytes)", path.display(), file_size);
    ...
    loop {
        ...
        // Send chunk: size (big-endian u32) + data
        let chunk_len = u32::try_from(n).unwrap_or(u32::MAX);
    ...
    // Read response
    let mut resp_buf = [0u8; SCAN_RESPONSE_BUFFER_SIZE];
