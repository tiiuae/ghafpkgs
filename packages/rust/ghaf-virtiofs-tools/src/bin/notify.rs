...
async fn handle_connection(
    ...
    while let Some(line) = lines.next_line().await? {
        ...
        trigger_refresh(base_path).await?;
        info!(
...
async fn trigger_refresh(dir: &Path) -> Result<()> {
    ...
    // Try to toggle the file's existence. Ignore race condition errors
    // (ENOENT for remove, EEXIST for create) since another concurrent
    // caller already did the work.
    if tokio::fs::try_exists(&tmp_file).await.unwrap_or(false) {
        match tokio::fs::remove_file(&tmp_file).await {
            Ok(()) => {}
            Err(e) if e.kind() == ErrorKind::NotFound => {
                debug!("Refresh file already removed by concurrent caller");
            }
            Err(e) => return Err(e).context("Failed to remove refresh file"),
        }
    } else {
        match tokio::fs::File::create(&tmp_file).await {
            Ok(_) => {}
            Err(e) if e.kind() == ErrorKind::AlreadyExists => {
                debug!("Refresh file already created by concurrent caller");
            }
            Err(e) => return Err(e).context("Failed to create refresh file"),
        }
    }
