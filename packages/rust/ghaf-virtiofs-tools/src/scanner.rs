...
/// Default `ClamAV` socket path.
pub const DEFAULT_CLAMAV_SOCKET: &str = "/var/run/clamav/clamd.ctl";
...
impl ClamAVScanner {
    /// Create a new scanner with the specified socket path.
    #[must_use]
    ...
    fn ping(&self) -> std::io::Result<String> {
    ...
    fn send_fd_for_scan(&self, fd: BorrowedFd<'_>) -> std::io::Result<String> {
        let mut stream = UnixStream::connect(&self.socket_path)?;
...
impl VirusScanner for ClamAVScanner {
    fn validate_availability(&self) -> Result<()> {
        let response = self.ping().map_err(|e| {
            anyhow::anyhow!("Failed to connect to ClamAV at {}: {e}", self.socket_path)
        })?;

        if response == "PONG" {
            info!("ClamAV daemon available: {}", self.socket_path);
            Ok(())
        } else {
            Err(anyhow::anyhow!(
