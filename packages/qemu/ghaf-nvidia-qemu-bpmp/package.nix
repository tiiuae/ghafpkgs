...
#
# ghaf-nvidia-qemu pinned to 10.1.5, plus two NVIDIA sync-forward guest bridges (sysbus
...
# QEMU (set via microvm.qemu.package); everything else stays on pkgs.ghaf-nvidia-qemu.
#
# 11.0.1 still lacks vfio-platform (checked 2026-07-21), so the pin holds. When
# base ghaf-nvidia-qemu bumps QEMU its patches may stop applying here -- fix the
...
#
# extraPatches/variantName let a variant (see ../ghaf-nvidia-qemu-bpmp-gpu) add patches
# and rename the binary. pkgs-by-name auto-calls with {}, so both default to the
# net-vm build -- pkgs.ghaf-nvidia-qemu-bpmp is byte-identical to before.
{
  lib,
  ghaf-nvidia-qemu,
  fetchurl,
...
ghaf-nvidia-qemu.overrideAttrs (
  _final: prev: rec {
    pname = "ghaf-nvidia-qemu-bpmp${variantName}";
    version = "10.1.5";
    ...
    # Drop nixpkgs' QEMU 11 TLS-test fix: the function it changes does not exist
    # in the pinned 10.1.5 source.
    #
    ...
    patches =
      (lib.filter (p: !(lib.hasInfix "fix-tls-tests-without-tasn1" (baseNameOf p))) (prev.patches or [ ]))
      ++ [
    ...
    postPatch = (prev.postPatch or "") + ''
      ...
      # ghaf-nvidia-qemu's 0006-ivshmem-flat-memory-support.patch is written against QEMU 11,
