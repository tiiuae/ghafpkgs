# SPDX-FileCopyrightText: 2022-2026 TII (SSRC) and the Ghaf contributors
# SPDX-License-Identifier: Apache-2.0
#
# Shared QEMU package implementation. NVIDIA builds omit the x86-only ACPI
# patches; x86 builds include them through withX86Acpi.
#
{
  lib,
  stdenv,
  qemu_kvm,
  packageName,
  withX86Acpi ? false,
  ...
}:
(qemu_kvm.override {
  # Display backends
  # sdlSupport = false;
  # gtkSupport = false;
  # vncSupport = false;
  # virglSupport = false;
  # ncursesSupport = false;
  # Audio backends
  # alsaSupport = false;
  # pulseSupport = false;
  # jackSupport = false;
  # Storage/peripheral
  # libiscsiSupport = false;
  # smartcardSupport = false;
  # Sphinx-based doc build fails (ninja step 1837); docs not needed in production
  enableDocs = false;
}).overrideAttrs
  (
    _final: prev: {
      pname = packageName;
      patches =
        prev.patches
        ++ [
          # Shared memory support for inter-VM communication
          ./patches/0006-ivshmem-flat-memory-support.patch
          # Increase TPM command timeout
          ./patches/0005-Increase-timeout-in-tpm_util_request.patch
          # USB host autoscan for bus/addr passthrough
          ./patches/0007-usb-host-enable-autoscan-for-bus-addr.patch
        ]
        ++ lib.optionals (withX86Acpi && stdenv.hostPlatform.isx86_64) [
          # ACPI battery/power management for VMs
          # https://github.com/blochl/qemu/pull/3
          # TODO: remove when merged upstream
          ./patches/0001-hw-acpi-Support-extended-GPE-handling-for-additional.patch
          ./patches/0002-hw-acpi-Introduce-the-QEMU-Battery.patch
          ./patches/0003-hw-acpi-Introduce-the-QEMU-AC-adapter.patch
          ./patches/0004-hw-acpi-Introduce-the-QEMU-lid-button.patch
        ];

      # qemu configures into ./build; nixpkgs no longer cds there before the build phases.
      postInstall = (prev.postInstall or "") + ''
        cp build/contrib/ivshmem-server/ivshmem-server $out/bin
      '';
    }
  )
