...
const gchar* standard_interfaces[] = {DBUS_INTERFACE_INTROSPECTABLE, DBUS_INTERFACE_PEER,
                                      DBUS_INTERFACE_PROPERTIES, nullptr};

static void proxy_return_error(GDBusMethodInvocation* invocation, GError* error) {
...
void handle_method_call_generic(GDBusConnection* connection, const gchar* sender,
    ...
    if (connection == proxy_state->target_bus) { // call comes from a client
        ...
        forward_bus_name = g_strdup(get_agent_name(object_path, interface_name, method_name));
        if (!forward_bus_name) {
            Log::error() << "No sender name found for connection, cannot forward method call";
        ...
        Log::verbose() << "Forwarding agent call to client: " << forward_bus_name;
    }
...
void update_object_with_new_interfaces(const gchar* object_path, GVariant* interfaces_dict) {
...
void on_service_vanished(GDBusConnection* connection G_GNUC_UNUSED, const gchar* name G_GNUC_UNUSED,
                         gpointer user_data G_GNUC_UNUSED) {
