...
int main(int argc, gchar* argv[]) {
    ...
    GOptionEntry entries[] = {
        ...
        {"source-object-path", 0, 0, G_OPTION_ARG_STRING, &config.source_object_path,
         "Object path of the source", "PATH"},
    ...
    if (opt_log_level) {
        if (!Log::setLevelFromString(opt_log_level)) {
    ...
    proxy_state->agents_registry = g_ptr_array_new_with_free_func(free_agent_callback_data);
    ...
    if (proxy_state->config.sni_mode) {
        ...
        auto sni =
            SniProxy::create(proxy_state->source_bus, proxy_state->target_bus,
                             proxy_state->config.target_bus_type, proxy_state->config.renamer_mode);
        if (!sni) {
