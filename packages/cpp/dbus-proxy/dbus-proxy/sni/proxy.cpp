...
const GDBusInterfaceVTable SniProxy::item_vtable_ = {
...
struct SniMethodCallContext {
  GDBusMethodInvocation *invocation =
      nullptr; // owned (caller must g_object_ref before storing)
...
static void forward_deferred_event(std::unique_ptr<SniDeferredEventCtx> ctx) {
  SniLog::info() << "[XDG] Forwarding dbusmenu.Event to "
                 << ctx->source_bus_name.c_str();
  ...
  if (ctx->invocation) {
    ...
    g_dbus_connection_call(ctx->source_bus, ctx->source_bus_name.c_str(),
                           ctx->dbusmenu_path.c_str(), DBUSMENU_INTERFACE,
                           "Event", ctx->event_params, nullptr,
                           G_DBUS_CALL_FLAGS_NONE, 10000, nullptr,
    ...
    g_dbus_connection_call(ctx->source_bus, ctx->source_bus_name.c_str(),
                           ctx->dbusmenu_path.c_str(), DBUSMENU_INTERFACE,
                           "Event", ctx->event_params, nullptr,
                           G_DBUS_CALL_FLAGS_NONE, 10000, nullptr, nullptr,
                           nullptr);
  }
...
static gboolean on_event_click_delay(gpointer user_data) {
  auto ctx = std::unique_ptr<SniDeferredEventCtx>(
      static_cast<SniDeferredEventCtx *>(user_data));
  SniLog::info() << "[XDG] Popup-close delay elapsed for "
                 << ctx->source_bus_name.c_str() << ": forwarding Event";
...
SniItem::~SniItem() {
  ...
  if (registered_objects && target_conn) {
    ...
    while (g_hash_table_iter_next(&iter, &key, nullptr))
      g_dbus_connection_unregister_object(target_conn.get(),
...
SniProxy::SniProxy(GDBusConnection *source_bus, GDBusConnection *target_bus)
  ...
  if (getenv("WAYLAND_DISPLAY")) {
    ...
    if (gtk_init_check()) {
      ...
      if (wayland_display_) {
        SniLog::info()
            << "GTK/GDK initialized on Wayland for XDG activation tokens";
      } else {
        SniLog::info() << "gtk_init_check succeeded but no default display";
      }
    } else {
      SniLog::info() << "gtk_init_check failed - XDG activation unavailable";
    }
  } else {
...
void SniProxy::try_lazy_wayland_init() {
  ...
  if (!getenv("WAYLAND_DISPLAY")) {
    ...
    g_autofree gchar *runtime_dir =
        g_strdup_printf("/run/user/%u", (unsigned)uid);
    ...
    if (dir) {
      ...
      while ((entry = g_dir_read_name(dir)) != nullptr) {
        ...
        if (g_str_has_prefix(entry, "wayland-") &&
            !g_str_has_suffix(entry, ".lock")) {
          g_autofree gchar *socket_path =
              g_strdup_printf("%s/%s", runtime_dir, entry);
          ...
          if (g_file_test(socket_path, G_FILE_TEST_EXISTS)) {
            ...
            SniLog::info() << "[XDG] Auto-detected Wayland socket: "
                           << socket_path;
            break;
          }
        }
      ...
      SniLog::info() << "[XDG] Cannot open " << runtime_dir << ": "
  ...
  if (!getenv("WAYLAND_DISPLAY")) {
    SniLog::info()
        << "[XDG] Lazy init: no Wayland socket found, XDG activation ";
    return;
  }
  ...
  if (gtk_init_check()) {
    ...
    if (wayland_display_)
      SniLog::info()
          << "[XDG] Lazy GTK/GDK initialized for XDG activation tokens";
    else
      SniLog::info() << "[XDG] Lazy gtk_init_check OK but no default display";
  } else {
    SniLog::info() << "[XDG] Lazy gtk_init_check failed";
  }
}
  ...
  if (sni_items_) {
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
      ...
      if (item->signal_subscription_id && source_bus_) {
        g_dbus_connection_signal_unsubscribe(source_bus_,
                                             item->signal_subscription_id);
...
char *SniProxy::resolve_unique_to_wellknown(const char *unique_name) {
  SniLog::info() << "[CALL] ListNames -> org.freedesktop.DBus";
  g_autoptr(GError) error = nullptr;
  g_autoptr(GVariant) result = g_dbus_connection_call_sync(
      source_bus_, DBUS_INTERFACE_DBUS, DBUS_OBJECT_PATH_DBUS,
      DBUS_INTERFACE_DBUS, "ListNames", nullptr, G_VARIANT_TYPE("(as)"),
      G_DBUS_CALL_FLAGS_NONE, 5000, nullptr, &error);

  if (!result) {
    SniLog::error() << "[REPLY] ListNames FAILED: "
  ...
  SniLog::info() << "[REPLY] ListNames returned names:";
  while (g_variant_iter_next(all_iter, "&s", &all_name)) {
    SniLog::info() << "  [" << total_count << "] " << all_name;
    total_count++;
  }
  g_variant_iter_free(all_iter);
  SniLog::info() << "[REPLY] ListNames total: " << total_count << " names";
  ...
  while (g_variant_iter_next(iter, "&s", &name)) {
    ...
    if (owner_result) {
      ...
      SniLog::info() << "[REPLY] GetNameOwner(" << name << ") = " << owner
                     << " (looking for " << unique_name << ")";
      if (g_strcmp0(owner, unique_name) == 0) {
        resolved = g_strdup(name);
        break;
      }
    } else {
      SniLog::error() << "[REPLY] GetNameOwner(" << name << ") FAILED: "
                      << (owner_error ? owner_error->message : "Unknown");
    }
  }
  ...
  if (sni_name_count == 0) {
    SniLog::info()
        << "ListNames returned NO org.kde.StatusNotifierItem-* names";
  } else {
    SniLog::info() << "ListNames returned " << sni_name_count << " SNI names";
  }

  if (resolved) {
    SniLog::info() << "Resolved " << unique_name << " -> " << resolved;
  } else {
    SniLog::info() << "No well-known SNI name found for " << unique_name;
  }
  return resolved;
}
...
void SniProxy::on_watcher_method_call(G_GNUC_UNUSED GDBusConnection *connection,
                                      const char *sender,
  ...
  if (g_strcmp0(method_name, "RegisterStatusNotifierItem") == 0) {
    ...
    SniLog::info() << "RegisterStatusNotifierItem from " << sender << ": "
                   << service;
    ...
    const char *service;
    g_variant_get(parameters, "(&s)", &service);
    SniLog::info() << "RegisterStatusNotifierHost from " << sender << ": "
                   << service;
    ...
    g_dbus_connection_emit_signal(
        self.source_bus_, nullptr, SNI_WATCHER_OBJECT_PATH,
        SNI_WATCHER_INTERFACE, "StatusNotifierHostRegistered", nullptr, &error);
    if (error) {
      SniLog::error() << "Failed to emit StatusNotifierHostRegistered: "
                      << error->message;
    }
    g_dbus_method_invocation_return_value(invocation, nullptr);
...
GVariant *SniProxy::on_watcher_get_property(
    G_GNUC_UNUSED GDBusConnection *connection, const char *sender,
    G_GNUC_UNUSED const char *object_path,
    G_GNUC_UNUSED const char *interface_name, const char *property_name,
    GError **error, gpointer user_data) {
  SniProxy &self = *static_cast<SniProxy *>(user_data);

  SniLog::info() << "[WATCHER] Get property '" << property_name << "' from "
                 << (sender ? sender : "(null)");
  ...
  if (g_strcmp0(property_name, "RegisteredStatusNotifierItems") == 0) {
    ...
    for (guint i = 0; i < self.registered_items_->len; i++) {
      const char *item_name = static_cast<const char *>(
          g_ptr_array_index(self.registered_items_, i));
      SniLog::info() << "[WATCHER]   RegisteredItems[" << i
                     << "] = " << item_name;
      g_variant_builder_add(&builder, "s", item_name);
    }
    SniLog::info() << "[WATCHER]   Total: " << self.registered_items_->len
                   << " items";
    return g_variant_builder_end(&builder);

  } else if (g_strcmp0(property_name, "IsStatusNotifierHostRegistered") == 0) {
    SniLog::info() << "[WATCHER]   = "
                   << (self.host_registered_ ? "true" : "false");
    return g_variant_new_boolean(self.host_registered_);

  } else if (g_strcmp0(property_name, "ProtocolVersion") == 0) {
    SniLog::info() << "[WATCHER]   = 0";
    return g_variant_new_int32(0);
  }

  SniLog::error() << "[WATCHER] Unknown property: " << property_name;
...
gboolean SniProxy::register_watcher() {
  ...
  if (!watcher_node_info_) {
    SniLog::error() << "Failed to parse watcher XML: "
                    << (error ? error->message : "Unknown");
    return FALSE;
  }
  ...
  if (watcher_reg_id_ == 0) {
    SniLog::error() << "Failed to register watcher: "
                    << (error ? error->message : "Unknown");
    return FALSE;
  }

  SniLog::info() << "Watcher registered at " << SNI_WATCHER_OBJECT_PATH;
  return TRUE;
}

// --- Watcher: name ownership callbacks ---

void SniProxy::on_watcher_name_acquired(G_GNUC_UNUSED GDBusConnection *conn,
                                        const gchar *name, gpointer user_data) {
  SniLog::info() << "Acquired watcher name: " << name;
  SniProxy &self = *static_cast<SniProxy *>(user_data);
  self.initial_scan();
}

void SniProxy::on_watcher_name_lost(G_GNUC_UNUSED GDBusConnection *conn,
                                    const gchar *name,
                                    G_GNUC_UNUSED gpointer user_data) {
  SniLog::error() << "Lost watcher name: " << name
                  << " -- another watcher running?";
}

gboolean SniProxy::own_watcher_name() {
  watcher_name_owner_id_ = g_bus_own_name_on_connection(
      source_bus_, SNI_WATCHER_BUS_NAME, G_BUS_NAME_OWNER_FLAGS_REPLACE,
      on_watcher_name_acquired, on_watcher_name_lost,
      this, // user_data
      nullptr);

  if (watcher_name_owner_id_ == 0) {
    SniLog::error() << "Failed to request watcher bus name";
    return FALSE;
  }
  return TRUE;
}
...
void SniProxy::method_reply_callback(GObject *source, GAsyncResult *res,
                                     gpointer user_data) {
  ...
  if (result) {
    ...
    if (truncated)
      result_str[300] = '\0';
    SniLog::info() << "[REPLY] Method call to " << ctx->forward_bus_name.c_str()
                   << " OK: " << result_str << (truncated ? "..." : "");
    g_dbus_method_invocation_return_value(ctx->invocation, result);
  } else {
    SniLog::error() << "[REPLY] Method call to "
                    << ctx->forward_bus_name.c_str()
...
void SniProxy::on_method_call(G_GNUC_UNUSED GDBusConnection *connection,
  ...
  g_autofree char *params_str =
      parameters ? g_variant_print(parameters, TRUE) : g_strdup("()");
  SniLog::info() << "[CALL] " << interface_name << "." << method_name
                 << params_str << " on " << object_path
  ...
  g_dbus_connection_call(fwd.proxy.source_bus_, fwd.source_bus_name.c_str(),
                         fwd.object_path.c_str(), interface_name, method_name,
                         parameters, nullptr, G_DBUS_CALL_FLAGS_NONE, 10000,
                         nullptr, method_reply_callback, ctx.release());
}
...
GVariant *SniProxy::on_get_property(G_GNUC_UNUSED GDBusConnection *connection,
  ...
  SniLog::info() << "[CALL] Get " << interface_name << "." << property_name
                 << " on " << object_path
                 << " -> dest=" << fwd.source_bus_name.c_str()
                 << " path=" << fwd.object_path.c_str();

  g_autoptr(GVariant) result = g_dbus_connection_call_sync(
      fwd.proxy.source_bus_, fwd.source_bus_name.c_str(),
      fwd.object_path.c_str(), DBUS_INTERFACE_PROPERTIES, "Get",
  ...
  if (result) {
    ...
    if (trunc)
      value_str[300] = '\0';
    SniLog::info() << "[REPLY] Get " << interface_name << "." << property_name
                   << " = " << value_str << (trunc ? "..." : "");
    return value;
  }

  SniLog::error() << "[REPLY] Get " << interface_name << "." << property_name
                  << " FAILED on " << fwd.object_path.c_str() << " ("
                  << fwd.source_bus_name.c_str() << "): "
...
void SniProxy::on_signal_received(G_GNUC_UNUSED GDBusConnection *connection,
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    if (candidate->ready &&
        g_hash_table_contains(candidate->proxied_objects, object_path)) {
      item = candidate;
      break;
    }
  }
  ...
  if (!item) {
    SniLog::verbose() << "[SIGNAL] " << interface_name << "." << signal_name
                      << " at " << object_path << " from " << sender_name
                      << " ignored (not proxied)";
    return;
  }
  ...
  g_autofree char *params_str =
      parameters ? g_variant_print(parameters, TRUE) : g_strdup("()");
  SniLog::info() << "[SIGNAL] Forwarding " << interface_name << "."
                 << signal_name << params_str << " at " << object_path
                 << " from " << sender_name << " -> target bus";

  g_autoptr(GError) error = nullptr;
  g_dbus_connection_emit_signal(item->target_conn.get(), nullptr, object_path,
                                interface_name, signal_name, parameters,
                                &error);
  if (error) {
    SniLog::error() << "[SIGNAL] Failed to forward " << interface_name << "."
...
gboolean SniProxy::register_path_with_xml(SniItem &item, const char *path,
                                          const char *xml) {
  ...
  g_autoptr(GDBusNodeInfo) node_info =
      g_dbus_node_info_new_for_xml(xml, &error);
  if (!node_info) {
    SniLog::error() << "Failed to parse hardcoded XML for " << path << ": "
                    << (error ? error->message : "Unknown");
    return FALSE;
  }
  ...
  if (node_info->interfaces) {
    for (int i = 0; node_info->interfaces[i]; i++) {
      ...
      auto ctx = std::make_unique<SniForwardContext>(path, item.source_bus_name,
                                                     *this);

      guint reg_id = g_dbus_connection_register_object(
          item.target_conn.get(), path, iface, &item_vtable_, ctx.get(),
          [](gpointer p) { delete static_cast<SniForwardContext *>(p); },
          &error);

      if (reg_id == 0) {
        SniLog::error() << "Failed to register " << iface->name << " at "
                        << path << " for " << item.source_bus_name.c_str()
                        << ": " << (error ? error->message : "Unknown");
        // ctx auto-deleted
        continue;
      }
      ...
      SniLog::info() << "Registered " << iface->name << " at " << path
                     << " for " << item.source_bus_name.c_str()
                     << " (id=" << reg_id << ")";
    }
  }

  if (registered_count > 0) {
    g_hash_table_insert(item.proxied_objects, g_strdup(path),
                        proxied_obj.release());
  }
  // else: proxied_obj auto-deleted

  char *cache_key =
      g_strdup_printf("%s:%s", item.source_bus_name.c_str(), path);
  g_hash_table_insert(item.node_info_cache, cache_key,
                      g_dbus_node_info_ref(node_info));
...
gboolean SniProxy::introspect_and_register_path(SniItem &item,
                                                const char *path) {
  if (g_hash_table_contains(item.proxied_objects, path))
    return TRUE;

  SniLog::info() << "[CALL] Introspect " << path << " -> "
                 << item.source_bus_name.c_str();
  g_autoptr(GError) error = nullptr;
  g_autoptr(GVariant) xml_variant = g_dbus_connection_call_sync(
      source_bus_, item.source_bus_name.c_str(), path,
      DBUS_INTERFACE_INTROSPECTABLE, "Introspect", nullptr,
      G_VARIANT_TYPE("(s)"), G_DBUS_CALL_FLAGS_NONE, 10000, nullptr, &error);

  if (!xml_variant) {
    SniLog::error() << "[REPLY] Introspect " << path << " on "
                    << item.source_bus_name.c_str()
                    << " FAILED: " << (error ? error->message : "Unknown");
    return FALSE;
  }
  ...
  SniLog::info() << "[REPLY] Introspect " << path << " on "
                 << item.source_bus_name.c_str() << " OK:\n"
                 << xml_data;
  ...
  if (!node_info) {
    SniLog::error() << "Failed to parse XML for " << path << " on "
                    << item.source_bus_name.c_str() << ": "
                    << (error ? error->message : "Unknown");
    return FALSE;
  }
...
void SniProxy::discover_menu_path(SniItem &item) {
  SniLog::info() << "[CALL] Get " << SNI_ITEM_INTERFACE << ".Menu on "
                 << SNI_ITEM_OBJECT_PATH
                 << " -> dest=" << item.source_bus_name.c_str();

  g_autoptr(GError) error = nullptr;
  g_autoptr(GVariant) result = g_dbus_connection_call_sync(
      source_bus_, item.source_bus_name.c_str(), SNI_ITEM_OBJECT_PATH,
      DBUS_INTERFACE_PROPERTIES, "Get",
      g_variant_new("(ss)", SNI_ITEM_INTERFACE, "Menu"), G_VARIANT_TYPE("(v)"),
      G_DBUS_CALL_FLAGS_NONE, 5000, nullptr, &error);

  if (!result) {
    SniLog::error() << "[REPLY] Get " << SNI_ITEM_INTERFACE
                    << ".Menu FAILED on " << item.source_bus_name.c_str()
                    << ": " << (error ? error->message : "Unknown");
    return;
  }
  ...
  g_variant_get(result, "(v)", &value);
  g_autofree char *value_str = g_variant_print(value, TRUE);
  SniLog::info() << "[REPLY] Get " << SNI_ITEM_INTERFACE
                 << ".Menu = " << value_str;
  ...
  if (g_variant_is_of_type(value, G_VARIANT_TYPE_OBJECT_PATH)) {
    ...
    if (menu_path && menu_path[0] == '/' && g_strcmp0(menu_path, "/") != 0 &&
      ...
      SniLog::info() << "Discovered menu path " << menu_path << " for "
                     << item.source_bus_name.c_str();
      introspect_and_register_path(item, menu_path);
    } else {
      SniLog::info() << "Menu path '" << (menu_path ? menu_path : "(null)")
                     << "' is not a valid menu, skipping";
    }
  } else {
    SniLog::info() << "Menu property is not an object path type, skipping";
  }
}

// --- Item name ownership callbacks ---

void SniProxy::on_item_name_acquired(G_GNUC_UNUSED GDBusConnection *conn,
                                     const gchar *name, gpointer user_data) {
  SniProxy &self = *static_cast<SniProxy *>(user_data);
  SniLog::info() << "Acquired name " << name << " on target bus";
  ...
  if (!item) {
    SniLog::error() << "SniItem for target name " << name << " not found";
    return;
  }
  ...
  if (!self.register_path_with_xml(*item, item->item_object_path.c_str(),
                                   SNI_ITEM_XML)) {
    SniLog::error() << "Failed to register SNI interface for "
                    << item->source_bus_name.c_str();
    return;
  }
  ...
  if (!self.register_path_with_xml(*item, DBUSMENU_PATH, DBUSMENU_ITEM_XML)) {
    SniLog::info() << "dbusmenu registration at " << DBUSMENU_PATH
                   << " failed for " << item->source_bus_name.c_str()
                   << " (non-fatal)";
  }
  ...
  item->signal_subscription_id = g_dbus_connection_signal_subscribe(
      self.source_bus_, item->source_bus_name.c_str(), nullptr, nullptr,
      nullptr, nullptr, G_DBUS_SIGNAL_FLAGS_NONE, on_signal_received,
  ...
  g_ptr_array_add(self.registered_items_,
                  g_strdup(item->source_bus_name.c_str()));

  // Emit StatusNotifierItemRegistered on source bus (for source-side tracking)
  g_autoptr(GError) error = nullptr;
  g_dbus_connection_emit_signal(
      self.source_bus_, nullptr, SNI_WATCHER_OBJECT_PATH, SNI_WATCHER_INTERFACE,
      SNI_SIGNAL_ITEM_REGISTERED,
      g_variant_new("(s)", item->source_bus_name.c_str()), &error);
  if (error) {
    SniLog::error() << "Failed to emit ItemRegistered for "
                    << item->source_bus_name.c_str() << ": " << error->message;
  }

  // Register with the target bus's existing watcher (the desktop panel)
  SniLog::info() << "[CALL] RegisterStatusNotifierItem('" << name << "') -> "
  ...
  g_dbus_connection_call(
      ...
      SNI_WATCHER_INTERFACE, "RegisterStatusNotifierItem",
      g_variant_new("(s)", name), nullptr, G_DBUS_CALL_FLAGS_NONE, 5000,
      nullptr,
      [](GObject *source, GAsyncResult *res, gpointer user_data) {
        ...
        if (result) {
          SniLog::info()
              << "[REPLY] RegisterStatusNotifierItem to target watcher OK";
          g_variant_unref(result);
        } else {
          SniLog::error()
              << "[REPLY] RegisterStatusNotifierItem to target watcher ";
          g_clear_error(&err);
        }
  ...
  SniLog::info() << "SNI item " << item->source_bus_name.c_str() << " -> "
                 << name << " fully proxied";
}

void SniProxy::on_item_name_lost(G_GNUC_UNUSED GDBusConnection *conn,
                                 const gchar *name,
                                 G_GNUC_UNUSED gpointer user_data) {
  SniLog::error() << "Lost name " << name << " on target bus";
}

// --- Discover and proxy a single SNI item ---

void SniProxy::discover_and_proxy_item(const char *bus_name,
                                       const char *object_path) {
  ...
  if (bus_name[0] == ':') {
    ...
    if (well_known) {
      if (!g_hash_table_contains(sni_items_, well_known)) {
        ...
        SniLog::verbose() << "SNI item " << well_known << " already tracked";
      }
      g_free(well_known);
      return;
    }
    ...
    SniLog::info() << "No well-known name for " << bus_name
                   << ", proxying with unique name";
    // Fall through to proxy directly.
  }

  if (g_hash_table_contains(sni_items_, bus_name)) {
    SniLog::verbose() << "SNI item " << bus_name << " already tracked";
    return;
  }

  SniLog::info() << "Discovering SNI item: " << bus_name;
  ...
  item->node_info_cache = g_hash_table_new_full(
  ...
  if (bus_name[0] == ':') {
    g_autofree gchar *gen_name = g_strdup_printf(
        "org.kde.StatusNotifierItem-proxy-%d-%d", getpid(), ++proxy_counter_);
  ...
  if (bus_name[0] != ':') {
    ...
    g_autoptr(GVariant) owner_result = g_dbus_connection_call_sync(
        source_bus_, DBUS_INTERFACE_DBUS, DBUS_OBJECT_PATH_DBUS,
        DBUS_INTERFACE_DBUS, "GetNameOwner", g_variant_new("(s)", bus_name),
        G_VARIANT_TYPE("(s)"), G_DBUS_CALL_FLAGS_NONE, 2000, nullptr,
        &owner_err);
    if (owner_result) {
      const char *unique_name;
      g_variant_get(owner_result, "(&s)", &unique_name);
      item->source_unique_name = unique_name;
      SniLog::info() << "Stored unique name for " << bus_name << ": "
                     << unique_name;
    } else {
      SniLog::info() << "Could not get unique name for " << bus_name << ": "
  ...
  if (!session_addr) {
    SniLog::error() << "Failed to get session bus address for " << bus_name
  ...
  if (!item->target_conn) {
    SniLog::error() << "Failed to create target connection for " << bus_name
                    << ": " << (conn_err ? conn_err->message : "Unknown");
    g_hash_table_remove(sni_items_, bus_name);
    return;
  }

  SniLog::info() << "Created per-item target connection for " << bus_name;

  // Own the target bus name on the per-item connection
  item->target_name_owner_id = g_bus_own_name_on_connection(
      item->target_conn.get(), item->target_bus_name.c_str(),
      G_BUS_NAME_OWNER_FLAGS_NONE, on_item_name_acquired, on_item_name_lost,
      this, // user_data = SniProxy*
      nullptr);

  if (item->target_name_owner_id == 0) {
    SniLog::error() << "Failed to request name "
                    << item->target_bus_name.c_str() << " on target bus";
    g_hash_table_remove(sni_items_, bus_name);
  }
}
...
void SniProxy::remove_item(const char *bus_name) {
  SniItem *item =
      static_cast<SniItem *>(g_hash_table_lookup(sni_items_, bus_name));
  if (!item) {
    SniLog::verbose() << "SNI item " << bus_name
                      << " not tracked, nothing to remove";
    return;
  }
  ...
  if (item->registered_objects && item->target_conn) {
    ...
    while (g_hash_table_iter_next(&iter, &key, nullptr)) {
      g_dbus_connection_unregister_object(item->target_conn.get(),
                                          GPOINTER_TO_UINT(key));
    }
  }

  SniLog::info() << "Removing target name " << item->target_bus_name.c_str()
                 << " (source=" << bus_name << ")";
  ...
  if (item->target_conn && !item->target_bus_name.empty() &&
    ...
    g_autoptr(GVariant) rn_result = g_dbus_connection_call_sync(
        ...
        g_variant_new("(s)", item->target_bus_name.c_str()),
    ...
    if (rn_result) {
      ...
      SniLog::info() << "ReleaseName(" << item->target_bus_name.c_str()
                     << ") = " << ret;
    } else {
      SniLog::error() << "ReleaseName(" << item->target_bus_name.c_str()
  ...
  if (item->target_conn) {
    ...
    SniLog::info() << "target_conn close for " << item->target_bus_name.c_str()
                   << ": " << (close_err ? close_err->message : "ok")
                   << " (is_closed="
                   << (g_dbus_connection_is_closed(item->target_conn.get())
                           ? "yes"
                           : "no")
  ...
  for (guint i = 0; i < registered_items_->len; i++) {
    if (g_strcmp0(
            static_cast<const char *>(g_ptr_array_index(registered_items_, i)),
            bus_name) == 0) {
  ...
  g_autoptr(GError) error = nullptr;
  g_dbus_connection_emit_signal(
      source_bus_, nullptr, SNI_WATCHER_OBJECT_PATH, SNI_WATCHER_INTERFACE,
      SNI_SIGNAL_ITEM_UNREGISTERED, g_variant_new("(s)", bus_name), &error);
  if (error) {
    SniLog::error() << "Failed to emit ItemUnregistered for " << bus_name
                    << ": " << error->message;
  }

  // Remove from hash table (triggers ~SniItem() via delete destroy callback)
  g_hash_table_remove(sni_items_, bus_name);
  SniLog::info() << "SNI item " << bus_name << " removed";
}
...
void SniProxy::on_name_owner_changed(G_GNUC_UNUSED GDBusConnection *connection,
  ...
  SniLog::verbose() << "[SIGNAL] NameOwnerChanged: name=" << " old_owner='"
                    << "' ";
  ...
  if (!g_str_has_prefix(name, SNI_ITEM_BUS_NAME_PREFIX)) {
    if (name[0] == ':' && old_owner[0] != '\0' && new_owner[0] == '\0') {
      ...
      for (guint i = 0; i < to_remove->len; i++) {
        const char *sname =
            static_cast<const char *>(g_ptr_array_index(to_remove, i));
        SniLog::info() << "App unique name " << name
                       << " vanished → removing stale item " << sname;
      ...
      if (g_hash_table_contains(self.sni_items_, name)) {
        SniLog::info() << "Directly proxied unique name " << name
                       << " vanished";
  ...
  if (appeared) {
    SniLog::info() << "SNI item appeared: " << name << " (owner: " << new_owner
                   << ")";
    ...
    if (g_hash_table_contains(self.pending_items_, new_owner)) {
      SniLog::info() << "Resolved pending item " << new_owner << " -> " << name;
      g_hash_table_remove(self.pending_items_, new_owner);
    }

    self.discover_and_proxy_item(name);
  } else if (vanished) {
    SniLog::info() << "SNI item vanished: " << name << " (was: " << old_owner
                   << ")";
    self.remove_item(name);
  }
}
...
gboolean SniProxy::subscribe_name_owner_changed() {
  name_owner_changed_sub_ = g_dbus_connection_signal_subscribe(
      source_bus_, DBUS_INTERFACE_DBUS, DBUS_INTERFACE_DBUS,
      DBUS_SIGNAL_NAME_OWNER_CHANGED, DBUS_OBJECT_PATH_DBUS, nullptr,
...
void SniProxy::initial_scan() {
  ...
  while (g_variant_iter_next(iter, "&s", &name)) {
    if (g_str_has_prefix(name, SNI_ITEM_BUS_NAME_PREFIX)) {
      SniLog::info() << "Found existing SNI item: " << name;
