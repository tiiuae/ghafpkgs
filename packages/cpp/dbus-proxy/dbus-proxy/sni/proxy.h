...
struct SniItem {
  std::string
      source_bus_name; // e.g. ":1.234" or "org.kde.StatusNotifierItem-1234-1"
  std::string
      source_unique_name; // unique name (:1.X) of the app that owns this item
  std::string
      target_bus_name; // name owned on target bus (may be generated proxy name)
...
struct SniForwardContext {
  ...
  class SniProxy
      &proxy; // back-reference to owning SniProxy (outlives this struct)
...
private:
  ...
  // --- Item lifecycle ---
  void discover_and_proxy_item(const char *bus_name,
                               const char *object_path = nullptr);
  void remove_item(const char *bus_name);
  void initial_scan();
  gboolean register_path_with_xml(SniItem &item, const char *path,
                                  const char *xml);
  ...
  static void
  on_watcher_method_call(GDBusConnection *connection, const char *sender,
                         const char *object_path, const char *interface_name,
                         const char *method_name, GVariant *parameters,
                         GDBusMethodInvocation *invocation, gpointer user_data);
  static GVariant *on_watcher_get_property(GDBusConnection *connection,
                                           const char *sender,
  ...
  static void on_watcher_name_acquired(GDBusConnection *conn, const gchar *name,
                                       gpointer user_data);
  static void on_watcher_name_lost(GDBusConnection *conn, const gchar *name,
                                   gpointer user_data);
  ...
  static void on_item_name_acquired(GDBusConnection *conn, const gchar *name,
                                    gpointer user_data);
  static void on_item_name_lost(GDBusConnection *conn, const gchar *name,
                                gpointer user_data);
  ...
  static constexpr const char *standard_interfaces_[] = {
      DBUS_INTERFACE_INTROSPECTABLE, DBUS_INTERFACE_PEER,
