...
class QEMULink:
        ...
        try:
            ...
            for x in res:
                for dev in x["devices"]:
                    ...
                    logger.info(
                        "  Description: %s. Class: %s. Bus: %s. Slot %s.",
                        class_info.get("desc"),
    ...
    async def add_usb_device(self, usb_info):
        if not wait_for_boot_qemu(self.socket_path, self.vm_boot_timeout, 0):
            logger.warning(
                "VM is not booted while adding device %s", usb_info.device_node
            )
    ...
    async def add_usb_device_by_vid_pid(self, usb_info):
        ...
        while True:
            ...
            try:
                ...
                if res:
                    logger.error(
                        "Failed to add device %s:%s with id %s: %s",
                        usb_info.vid,
                        usb_info.pid,
                        qemuid,
                        res,
                    ...
                    logger.info(
                        "Attached USB device %s:%s with id %s",
                        usb_info.vid,
                        usb_info.pid,
                        qemuid,
                    )
                ...
                if str(e).startswith("Duplicate device ID"):
                    logger.info(
                        "USB device %s:%s with id %s is already attached to the VM",
                        usb_info.vid,
                        usb_info.pid,
                        qemuid,
                    )
                    return
                logger.warning(
                    "Failed to add USB device %s:%s with id %s: %s",
                    usb_info.vid,
                    usb_info.pid,
                    qemuid,
                    e,
            ...
            if i < self.vm_retry_count:
                ...
                break
        logger.error(
            "Failed to add USB device %s:%s with id %s after %s attempts",
            usb_info.vid,
            usb_info.pid,
            qemuid,
            i,
    ...
    async def remove_usb_device(self, usb_info):
        ...
        try:
            ...
            if str(e) == f"Device '{qemuid}' not found":
                logger.debug(
                    "Failed to remove USB device %s from %s: %s",
                    qemuid,
                    self.socket_path,
                    e,
                )
    ...
    async def remove_evdev_device(self, device):
        logger.debug(
            "Removing evdev device %s with id %s", device.device_node, device.sys_name
        )
        qmp = QMPClient()
        try:
            await qmp.connect(self.socket_path)
