...
def is_sound_device(device):
    return (
        ...
        and device.device_type != "pcm"
        and device.sys_name.startswith("card")
    )
...
def is_network_device(device):
    ...
    return (
        ...
        and device.device_type != "bridge"
        and driver != "tun"
        and driver != "bridge"
        and device.sys_name != "lo"
    )
...
def is_smartcard(device):
    return (
        ...
        and device.properties.get("ID_SMARTCARD_READER") == "1"
    )
            ...
            for device in context.list_devices(subsystem="input"):
                ...
                if is_input_device(device) and bus != "usb":
                    ...
                    if await test_grab(device):
                        logger.info(
                            "The device is grabbed by another process, it is likely already connected to the VM"
                        )
                    else:
    ...
    for device in context.list_devices(subsystem="usb"):
        if is_usb_device(device):
            ...
            if is_usb_hub(usb_info.interfaces):
                logger.debug(
                    "USB device %s:%s is a USB hub, skipping",
                    usb_info.vid,
                    usb_info.pid,
                )
                continue
            try:
                vm = await vm_for_usb_device(
                    context, config, None, usb_info, None, False
                )
                if vm:
